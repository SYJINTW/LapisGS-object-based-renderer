...
# TRAINING_CAM_RES_LIST=("1" "2" "4" "8")
# CAM_DISTANCE_LIST=("1" "2" "3" "4" "5" "6" "7" "8")
# GS_RES_LIST=("1" "2" "4" "8")

TRAINING_CAM_RES_LIST=("8")
GS_RES_LIST=("8")
CAM_DISTANCE_LIST=("1" "2" "3" "4" "5" "6" "7" "8")

for TRAINING_CAM_RES in "${TRAINING_CAM_RES_LIST[@]}"; do
    for CAM_DISTANCE in "${CAM_DISTANCE_LIST[@]}"; do
        for GS_RES in "${GS_RES_LIST[@]}"; do

            # # Only run if TRAINING_CAM_RES >= GS_RES
            # if [ "$TRAINING_CAM_RES" -ge "$GS_RES" ]; then

            # Only run if TRAINING_CAM_RES is equal to GS_RES
            # if [ "$TRAINING_CAM_RES" -eq "$GS_RES" ]; then

                TRAINING_DIR="${TRAINING_DATA_BASE_DIR}/${SCENE_NAME}"
                OUTPUT_DIR="${OUTPUT_DATA_BASE_DIR}/${SCENE_NAME}/opacity/${SCENE_NAME}_res${TRAINING_CAM_RES}"
                RESULTS_NAME="ours_${CAM_DISTANCE}x_res${GS_RES}"
                OUR_CAM_PATH="${OUR_CAM_BASE_DIR}/results_test_${CAM_DISTANCE}x/transforms.json"
                ...
                python ./metrics.py \
                    ...
                    --mask_dir "${OUR_CAM_BASE_DIR}/results_test_${CAM_DISTANCE}x/mask"
